...
(function ($) {
  ...
  jQuery.validator.addMethod('postcodeUK', function (value, element) {
    error_check_on_search_form(element); //console.log(element.parentElement.classList.toggle('has-error','has-feedback'));
    //return this.optional(element) || /^[A-Z]{1,2}[0-9]{1,2} ?[0-9][A-Z]{2}$/i.test(value);

    return this.optional(element) || /^[A-Z]{1,2}[0-9]{1,2}|[A-Z]{1,2}[0-9]{1}[A-Z]{1} ?[0-9][A-Z]{2}$/i.test(value); //return this.optional(element) || /^([A-Z][A-Z0-9]?[A-Z0-9] ?[A-Z0-9] ?{1,2}[0-9][A-Z0-9]{2})$/i.test(value);
  ...
  jQuery('#geocode').validate({
    ...
    submitHandler: function submitHandler(form) {
      ...
      var postcode = postcode.replace(/\s+/g, ''); //var geocoder = new google.maps.Geocoder();
      ...
      var onSuccess = function onSuccess(results, status) {
